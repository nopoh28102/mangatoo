...
from utils_dynamic_urls import safe_redirect_url
...
def admin_add_cloudinary_account():
    ...
    try:
        ...
        new_account = CloudinaryAccount(
            ...
            api_key=api_key,
            api_secret=api_secret,
            storage_used_mb=storage_used / (1024 * 1024),  # Convert to MB
            storage_limit_mb=storage_limit / (1024 * 1024),  # Convert to MB
            plan_type=plan_type,
            is_active=True,
            is_primary=(CloudinaryAccount.query.count() == 0),  # First account is primary
            priority_order=CloudinaryAccount.query.count() + 1
        )
        ...
        try:
            ...
            try:
                if upload_method == 'images':
                    ...
                    # ZIP file upload using ConvertAPI - user must use the preview/extract feature first
                    flash('يجب استخدام ميزة المعاينة واستخراج الصور من ZIP أولاً', 'warning')
                    db.session.rollback()
                    return safe_redirect(request.url)
...
def admin_extract_zip_preview():
    """Extract ZIP file and return preview data with background processing"""
    if not (current_user.is_admin or current_user.is_publisher):
        return jsonify({'success': False, 'error': 'Unauthorized'}), 403
    
    try:
        # Check if ZIP file is provided
        if 'chapter_zip' not in request.files:
        ...
        max_size_mb = 200  # 200MB max file size for local processing
        max_size = max_size_mb * 1024 * 1024  # Convert MB to bytes
        
        if content_length > max_size:
            return jsonify({
                'success': False, 
                'error': f'File too large. Maximum size is {max_size_mb}MB, got {content_length / (1024*1024):.1f}MB'
            })
        
        logging.info(f"🔄 Starting ZIP extraction for {zip_file.filename} ({content_length / (1024*1024):.1f}MB)")
        
        # Use local ZIP extraction only
        result = extract_zip_locally(zip_file)
        
        if result['success']:
            logging.info(f"✅ Local ZIP extraction completed: {result['total']} images processed")
        ...
        return jsonify({
            'success': False, 
            'error': f'ZIP extraction failed: {error_message}'
        })


def extract_zip_locally(zip_file):
    """Extract ZIP file locally without using ConvertAPI - faster and no quota usage"""
    try:
        import zipfile
        import tempfile
        import base64
        ...
        import io
        from utils_settings import SettingsManager
        from stream_unzip import stream_unzip  # For large ZIP files
        ...
        logging.info(f"📦 Extracting ZIP file locally: {zip_file.filename}")
        ...
        with zipfile.ZipFile(zip_path, 'r') as zip_ref:
            ...
            for i, filename in enumerate(image_files):
                ...
                try:
                    ...
                    processed_count += 1
                    logging.info(f"✅ Processed: {os.path.basename(filename)} ({size_str})")
        ...
        return {
            ...
            'message': f'تم استخراج {len(extracted_images)} صورة من الأرشيف بنجاح (معالجة محلية)'
        }
...
def test_database_connection_internal(database_url):
    ...
    try:
        ...
        # Parse the database URL to get info
