...
def safe_redirect(url):
    ...
    # Only allow same-origin redirects (no external domains)
...
try:
    from utils_settings import SettingsManager as RealSettingsManager
...
def read_chapter_view(chapter):
    ...
    # Process image URLs for each page (prioritize Cloudinary)
    for page in pages:
        if hasattr(page, 'cloudinary_url') and page.cloudinary_url:
            # Use Cloudinary URL directly - already optimized and global
            page.image_url = page.cloudinary_url
        elif hasattr(page, 'image_path') and page.image_path:
            # Fallback for legacy images
            if page.image_path.startswith('http'):
                page.image_url = page.image_path
            else:
                page.image_url = url_for('static', filename=page.image_path)
        else:
            # Last resort fallback
            page.image_url = url_for('static', filename=f'uploads/manga/{manga.id}/{chapter.id}/page_{page.page_number:03d}.jpg')
...
def admin_settings():
    ...
    cloudinary_stats = {}
    try:
        from models_cloudinary import CloudinaryAccount, CloudinaryAccountManager
        accounts = CloudinaryAccount.query.order_by(CloudinaryAccount.priority_order).all()
        cloudinary_accounts = accounts
        cloudinary_stats = CloudinaryAccountManager.get_account_statistics()
    except Exception as e:
        import logging
...
def admin_upload_logo():
    ...
    try:
        ...
        if '.' not in (file.filename or '') or \
           (file.filename and file.filename.rsplit('.', 1)[1].lower() not in allowed_extensions):
        ...
        if hasattr(file, 'content_length') and file.content_length > 5 * 1024 * 1024:
            return jsonify({'success': False, 'message': 'حجم الملف كبير جداً'})
        
        # Create uploads directory if it doesn't exist
        upload_dir = os.path.join(app.static_folder or 'static', 'uploads', 'logos')
        os.makedirs(upload_dir, exist_ok=True)
        ...
        # Generate URL
        logo_url = url_for('static', filename=f'uploads/logos/{filename}')
...
def admin_seo_preview():
    ...
    from utils_seo import generate_complete_seo_data
...
def upload_logo():
    ...
    try:
        ...
        SettingsManager.set('logo_url', logo_url, data_type='string', category='general', 
            ...
            try:
                if upload_method == 'images':
                    ...
                    if has_tested_data:
                        ...
                        try:
                            from simple_manga_scraper import scrape_olympustaff_simple as scrape_olympustaff_chapter
                        except ImportError:
                ...
                page_count = successful_uploads if 'successful_uploads' in locals() else 0
                print(f"✅ تم إنشاء الفصل {chapter_number} مع {page_count} صفحة وبدء رفعها في الخلفية")
                flash(f'تم إضافة الفصل {chapter_number} بنجاح مع {page_count} صورة!', 'success')
                return redirect(url_for('manga_detail', manga_id=manga.id))
        ...
        try:
            ...
            try:
                if upload_method == 'images':
                    ...
                    from background_uploader import background_uploader
                    ...
                    temp_folder, saved_files = background_uploader.save_temp_images_from_files(chapter_files, manga.id, chapter.id)
                    ...
                    try:
                        ...
                        upload_temp_path = background_uploader.temp_storage_path / upload_temp_folder
                    ...
                    if scraping_tested == 'true' and scraped_images_json:
                        try:
                            ...
                            print(f"✅ تم رفع {successful_uploads} صورة مكشوطة إلى Cloudinary بنجاح")
                    ...
                    raise Exception('طريقة رفع غير صحيحة')
                ...
                # تعيين معلومات الفصل - سيتم تحديث page_count عند انتهاء الرفع في الخلفية
                chapter.page_count = successful_uploads
                chapter.is_available = False  # سيتم تفعيله عند انتهاء رفع الصور
            ...
            # Add categories if selected using safe method
...
def admin_cloudinary_add_account():
    ...
    try:
        from models_cloudinary import CloudinaryAccountManager
        ...
        storage_limit = float(request.form.get('storage_limit', 25600))
