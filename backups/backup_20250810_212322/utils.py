...
def process_chapter_upload(manga_id, chapter_file, chapter_number):
    ...
    try:
        ...
        chapter = Chapter(
            ...
            pages=0  # Will be updated after processing images
        )
        ...
        for i, image_path in enumerate(image_files):
            ...
            page = PageImage(
                ...
                page_number=i + 1,
                image_path=os.path.join(app.config['UPLOAD_FOLDER'], rel_path),
                width=width or 0,
                height=height or 0
            )
            db.session.add(page)
...
def create_default_categories():
    ...
    for name, name_ar, description in default_categories:
        if not Category.query.filter_by(name=name).first():
            category = Category(name=name, name_ar=name_ar, description=description)
