...
class Manga(db.Model):
    ...
    def generate_slug(self):
        ...
        base_slug = slug
        counter = 1
        while Manga.query.filter_by(slug=slug).filter(Manga.id != self.id).first():
...
class Chapter(db.Model):
    ...
    publisher_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=True)  # Who published this chapter
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    # Scheduling fields
...
class ReadingProgress(db.Model):
    ...
    manga = db.relationship('Manga', backref='progress_records')
    chapter = db.relationship('Chapter', backref='progress_records')
...
class ScrapingLog(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    source_id = db.Column(db.Integer, db.ForeignKey('auto_scraping_source.id'), nullable=False)
